...
import wikipedia.WikipediaRanking.isFound
...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(l => {
      val count=occurrencesOfLang(l,rdd)
      (l,count)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(data => (data._1, data._2.size)).collect().toList.sortBy(_._2).reverse
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val list=rdd.flatMap( articile => for (lang <- langs if isFound(articile,lang) ==1) yield (lang,1))
