...
class StackOverflow extends StackOverflowInterface with Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    val ans = postings.filter(_.postingType == 2).map(p => (p.parentId.get, p))
    (questions join ans).groupByKey()
  }
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.mapValues(v => (v.head._1, answerHighScore(v.map(_._2).toArray))).values
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(pair => (firstLangInTag(pair._1.tags, langs), pair._2)).filter(_._1.isDefined).map(p => (p._1.get * langSpread, p._2)).
      persist()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    var newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = Map()
    // Fill in the newMeans array
    vectors.map(vec => (findClosest(vec, means), vec)).groupByKey().mapValues(s => averageVectors(s)).collect().
      foreach(p => newMeansMap= newMeansMap + (p))
